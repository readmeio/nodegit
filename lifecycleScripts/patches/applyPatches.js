...
module.exports = function applyPatches() {
  ...
  if (fs.existsSync(zutilPath)) {
    ...
    if (zutilContent.includes("Don't redefine fdopen on macOS - system headers already provide it")) {
  ...
  return exec('git apply --directory=vendor/libgit2 "' + node22Patch + '"', {
    ...
    console.log("[nodegit] Successfully applied Node.js 22 compatibility patch");
  }).catch(function(error) {
    console.warn("[nodegit] WARNING - Failed to apply Node.js 22 compatibility patch:", error.message);
